    ...
    <div className="rp-chart">
      <div className="rp-chart__bars">
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="rp-chart__col"
...
// ── Horizontal bar list ──────────────────────────────────────
// Dùng item.label (backend giờ trả "label" cho cả ngôn ngữ lẫn thiết bị)
function HBarList({ items, color = "var(--color-primary,#6366f1)" }) {
  return (
    <div className="rp-hbar-list">
      {items.map((item, i) => (
        <div key={i} className="rp-hbar-item">
          <span className="rp-hbar-label">{(item.label || item.languageCode || item.deviceType || "—").toUpperCase()}</span>
          <div className="rp-hbar-track">
            <div className="rp-hbar-fill" style={{ width: `${item.pct}%`, background: color }}/>
          </div>
          <span className="rp-hbar-count">{item.count.toLocaleString()}</span>
...
const SkeletonBlock = ({ h = 180 }) => <div className="rp-skeleton" style={{ height: h }}/>;
...
export default function ReportPage() {
  ...
  // Fetch tất cả báo cáo khi filter thay đổi
    ...
    <Layout>
      <div className="rp-page">
        ...
        <div className="rp-header">
          <div className="rp-header__left">
            <button className="rp-back-btn" onClick={() => navigate("/admin/dashboard")}>
              <span style={{ transform: "scaleX(-1)", display:"inline-block" }}>›</span> Dashboard
            </button>
        ...
        <div className="rp-filters">
          ...
          <button className="rp-btn rp-btn--outline" onClick={fetchAll}>
            🔄 Làm mới
          </button>
        </div>
        ...
        <div className="rp-stats">
          {loadingSummary ? (
            <><div className="rp-stat-skeleton"/><div className="rp-stat-skeleton"/><div className="rp-stat-skeleton"/></>
          ) : summary ? (
            <>
              <StatCard icon={<IconHeadphone />} label="Tổng lượt nghe"
                value={(summary.total ?? 0).toLocaleString()}
                sub={`TB ${summary.avgDurationSec ?? 0}s / lượt`}
                color="blue" />
              <StatCard icon={<IconBarChart />} label="TB / ngày"
                value={(summary.avgPerDay ?? 0).toLocaleString()}
                sub={`Trong khoảng ${range === "today" ? "hôm nay" : range === "month" ? "30 ngày" : "7 ngày"}`}
                color="purple" />
              <StatCard icon={<IconGlobe />} label="Ngôn ngữ phổ biến"
                value={summary.topLanguage ?? "—"}
                sub={`${summary.totalLanguages ?? 0} ngôn ngữ`}
                color="green" />
            </>
          ) : null}
        </div>
        ...
        <div className="rp-card">
          <div className="rp-card__head">
            <div className="rp-card__title-wrap"><IconBarChart /><h2 className="rp-card__title">Lượt nghe theo ngày</h2></div>
            <span className="rp-card__sub">Tổng: <strong>{totalListens.toLocaleString()}</strong> lượt</span>
          </div>
          {loadingChart
            ? <SkeletonBlock h={200}/>
            : chartData.length === 0
        ...
        <div className="rp-grid-2">
          <div className="rp-card">
            <div className="rp-card__head">
              <div className="rp-card__title-wrap"><IconGlobe /><h2 className="rp-card__title">Theo ngôn ngữ</h2></div>
            </div>
            {loadingLang
              ? <SkeletonBlock h={140}/>
              : langData.length === 0
                ? <p className="rp-empty">Chưa có dữ liệu.</p>
                : <HBarList items={langData} color="#6366f1"/>}
          </div>
          <div className="rp-card">
            <div className="rp-card__head">
              <div className="rp-card__title-wrap"><IconPhone /><h2 className="rp-card__title">Theo thiết bị</h2></div>
            </div>
            {loadingDevice
              ? <SkeletonBlock h={140}/>
              : deviceData.length === 0
                ? <p className="rp-empty">Chưa có dữ liệu.</p>
                : <HBarList items={deviceData} color="#8b5cf6"/>}
          </div>
        </div>
        ...
        <div className="rp-card rp-card--full">
          <div className="rp-card__head">
            <div className="rp-card__title-wrap"><IconList /><h2 className="rp-card__title">Chi tiết theo gian hàng</h2></div>
          </div>
          {loadingBooth ? (
            <div className="rp-skeleton-list">
              {[0,1,2,3].map(i => <div key={i} className="rp-skeleton-row"/>)}
            </div>
          ) : boothData.length === 0 ? (
            <p className="rp-empty">Chưa có dữ liệu.</p>
          ) : (
            <table className="rp-table">
              <thead>
                <tr>
                  ...
                  <th>Gian hàng</th>
                  <th>Sự kiện</th>
                  <th><span className="rp-th-icon"><IconHeadphone/></span> Lượt nghe</th>
                  <th><span className="rp-th-icon"><IconClock/></span> TB thời lượng</th>
                  <th>Tỷ lệ</th>
                </tr>
              </thead>
              <tbody>
